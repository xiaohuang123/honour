...
(function(factory){
  ...
  dialog.init = function(opt, ok, no){
    ...
    _this.dom.box.on('click', 'a', function(){
      ...
      if (_rev === 'close') {
        ...
        if(typeof(_this.closeCache)==="function"){
            _this.closeCache();
        }
        _this.exit();
      } else {
      	if (_rev === 'btn_ok' || _rev === 'btn_no') {
      	    if($(this).text()=='导出帐号名单'){
                $("#cxdialog").attr({"position":"fixed","top":"50%","left":"50%"});
            } else {
              /*$("#cxdialog").attr("style","");*/
            }
  ...
  dialog.setContent = function(opt){
    ...
    if (typeof opt.ok === 'function') {
      ...
      if (typeof opt.closeBtnFun === 'function') {
          _this.closeCache = opt.closeBtnFun;
      };
